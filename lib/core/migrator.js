...
async function getMigrator(type, args) {
  ...
  let migratorPath = _index.default.path.getPath(type);

  if (type === 'migration') {
    migratorPath = _index.default.path.getMigrationsCompiledPath();
  }

  if (type === 'seeder') {
    migratorPath = _index.default.path.getSeedersCompiledPath();
  }
